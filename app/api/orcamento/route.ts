...
export async function POST(request: Request) {
  ...
  if (!parsed.ok) {
    logWarn('validation_error', 'Payload inválido em /api/orcamento', {
  ...
  try {
    const protocol = generateProtocol('ORC')

    logInfo('lead_submit_success', 'Lead de orçamento recebido', {
      endpoint: '/api/orcamento',
      protocol,
    ...
    const response: LeadApiResponse = {
      success: true,
      protocol,
      message: 'Orçamento solicitado com sucesso. Nossa equipe entrará em contato.',
    }

    return NextResponse.json(response, { status: 200 })
  } catch (error) {
    logError('lead_submit_error', 'Falha ao processar lead de orçamento', {
      endpoint: '/api/orcamento',
    ...
    const response: LeadApiResponse = {
      success: false,
      message: 'Não foi possível processar seu orçamento agora. Tente novamente em instantes.',
    }
